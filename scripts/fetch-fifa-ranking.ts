// Download the full FIFA/Coca-Cola Men's World Ranking and emit src/data/fifa-ranking.ts.
//
// Usage:
//     node scripts/fetch-fifa-ranking.ts
//
// The script scrapes the complete 211-team ranking table (rank, team, points)
// from whereig.com, which mirrors the official FIFA release, using only built-ins.
// It then writes a TypeScript module to src/data/fifa-ranking.ts.
//
...
function parseRanking(html: string): RankingRow[] {
  ...
  // That includes a "Top 20" summary plus per-group standings tables.
  // The one we want is the full 211-row table, so parse every candidate and keep the largest.
  let best: RankingRow[] = []
  let searchFrom = 0
  for (;;) {
...
function emitTypescript(rows: RankingRow[], outPath: string): void {
  const lines = [
    ...
    '// That is the last release before the 2026 World Cup.',
    '// All 211 member associations in rank order, with their ranking points.',
    '//',
