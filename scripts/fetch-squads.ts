// Download the 2026 FIFA World Cup squads from Wikipedia and emit src/data/squads.ts.
//
// Usage:
...
// Canonical team order, matching the order in src/data/teams.ts.
const TEAM_ORDER = [
  'mex',
  'rsa',
  'kor',
...
function stripWiki(s: string): string {
  ...
  // Bold / italic
  s = s.replace(/'''?/g, '')
  // XML refs, looped until stable so adversarial or nested <ref> markup can't survive a single pass.
  // See stripTags in fetch-fifa-ranking.ts.
  let prev: string
  do {
    prev = s
    s = s.replace(/<ref[^>]*\/>/g, '')
...
function emitTypescript(squads: Map<string, Player[]>, outPath: string): void {
  const lines = [
    '// 2026 FIFA World Cup squads, with 48 teams and 26 players each.',
