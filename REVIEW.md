...
and the same 6-line `<ScoreDialog v-if>` block (~20 lines × 3). The app already solved
this exact problem for TeamDialog with `provideTeamViewer`.
...
`use-match-result-form`, `use-possible-teams-dialog`, `KnockoutView`, `SettingsView`.
Meanwhile `GroupStandingsTable`/`StandingsRow`/`MatchCard` are properly dumb. Leaf
renderers like `OriginColumn` and `ThirdPlaceTable` pull global state and run domain
computations themselves, making them untestable without Pinia and unreusable.
**Fix:** keep store access in views + container components (BracketView, GroupTable);
...
and per `resolveTeamRef` chain in `BracketView` — a single score entry on the groups
page recomputes standings ~25+ times; a full bracket render performs on the order of
10³ redundant calls (`knockout.ts:23-62`, `third-place.ts:91-106`). Trivial at this data
size, but structural — and `possible-teams` sits on the same primitive (§4.1).
**Fix:** a store getter `standingsByGroup = computed(() => new Map(GROUP_IDS.map(...)))`
gives Vue-cached invalidation for the views; fingerprint-memoize
...
Also, `useBracketConnectors` holds no reactive state and registers no lifecycle — the
`use` prefix over-promises. Its `viewEl` parameter is only ever null-checked, never used
in any computation, and exists mostly to justify a template ref in `BracketView`.
**Fix:** import the maps directly from `../lib/bracket-graph`, delete the re-export,
drop the `viewEl` parameter and the `bracketViewEl` template ref. Either give the
...
unchanged in both intermediate layers (`BracketRound.vue`, `BracketMatchItem.vue`) — 10
emit declarations plus 10 template forwarders carrying zero logic, with highlight props
drilled alongside. `BracketMatchItem` is a 71-line component whose entire job is a
wrapper div with hover listeners plus an optional label; it re-declares every `MatchRow`
field as a prop just to receive `v-bind="row"`.
**Fix:** fold `BracketMatchItem` into `BracketRound` (~8 lines of template); if the
remaining chatter still grates, `provide` the highlight controller from BracketView —
5 callbacks + 2 state fields is exactly the component-scoped-store case provide/inject
exists for.
