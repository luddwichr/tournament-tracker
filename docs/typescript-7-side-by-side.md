...
This project compiles with TypeScript 7 (the native, Go-based compiler) but
keeps TypeScript 6 installed alongside it, following the setup from the
...
This file documents why, what the workarounds are, and when they can be
removed.
...
| ------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typescript-eslint` | Peer dependency `typescript@">=4.8.4 <6.1.0"` fails the install (`ERESOLVE`); typed lint rules need the TS JS API    | [typescript-eslint/typescript-eslint#10940](https://github.com/typescript-eslint/typescript-eslint/issues/10940) (labeled "blocked by external API"; [#12518](https://github.com/typescript-eslint/typescript-eslint/issues/12518) was closed as a duplicate of it) |
...
- `"@typescript/native": "npm:typescript@7.…"` — the real TypeScript 7;
  owns the `tsc` binary. `npx tsc` is the fast native compiler
  (measured ~7–8× faster than `tsc6` on this codebase's vue-free sources).
