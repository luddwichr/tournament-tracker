 ...
 * document loaded so `localStorage.removeItem` has somewhere to run, followed by a second real `goto(...)`.
 */
 ...
 * context-level init script, so the app picks them up on its one real `goto(...)`.
 * This avoids a throwaway `goto('/')` just to get a document loaded so a plain `page.evaluate` calling
 * `localStorage.setItem` has somewhere to run.
 */
