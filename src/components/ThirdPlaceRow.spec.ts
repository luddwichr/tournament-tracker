...
describe('ThirdPlaceRow', () => {
  it('renders the tiebreaker columns in breaking order: points, GD, goals, fair-play, FIFA rank', () => {
    const wrapper = mount(ThirdPlaceRow, { props: { final: false, rank: 1, stat: makeStat() } })
  ...
  it('renders negative goal difference without a plus sign', () => {
    const wrapper = mount(ThirdPlaceRow, { props: { final: false, rank: 1, stat: makeStat({ goalDiff: -2 }) } })
  ...
  it('shows "safe" status for rank <= 8 while the group stage is not final', () => {
    const wrapper = mount(ThirdPlaceRow, { props: { final: false, rank: 8, stat: makeStat() } })
  ...
  it('includes a visually-hidden rank, group and status label when played > 0', () => {
    const wrapper = mount(ThirdPlaceRow, { props: { final: true, rank: 1, stat: makeStat() } })
    expect(wrapper.find('.visually-hidden').text()).toBe('Platz 1, Gruppe A, qualifiziert')
  })

  it('still names the rank and group in the visually-hidden label when played === 0', () => {
    const wrapper = mount(ThirdPlaceRow, {
      props: { final: false, rank: 1, stat: makeStat({ form: [], played: 0 }) },
    })
    expect(wrapper.find('.visually-hidden').text()).toBe('Platz 1, Gruppe A')
  })
})
